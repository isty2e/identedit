...
#[test]
fn select_covers_javascript_typescript_extensions_and_kinds() {
    let js_file = fixture_path("example.js");
    let jsx_file = fixture_path("example.jsx");
    let ts_file = fixture_path("example.ts");
    let tsx_file = fixture_path("example.tsx");

    assert_select_kind_and_optional_name(&js_file, "function_declaration", Some("processData"));
    assert_select_kind_and_optional_name(&jsx_file, "function_declaration", Some("View"));
    assert_select_kind_and_optional_name(&ts_file, "function_declaration", Some("processData"));
    assert_select_kind_and_optional_name(&tsx_file, "function_declaration", Some("View"));

    assert_select_kind_and_optional_name(&js_file, "class_declaration", Some("Processor"));
    assert_select_kind_and_optional_name(&js_file, "arrow_function", None);
}

#[test]
fn transform_replace_and_apply_support_typescript_arrow_function() {
    let file_path = copy_fixture_to_temp("example.ts", ".ts");
    ...
    assert!(modified.contains("=> value - 1"));
}

#[test]
fn select_transform_apply_pipeline_supports_tsx_function_rewrite() {
    let file_path = copy_fixture_to_temp("example.tsx", ".tsx");
    let select_output = run_identedit(&[
        "select",
        "--kind",
    ...
    let modified = fs::read_to_string(&file_path).expect("file should be readable");
    assert!(modified.contains("<section>Updated</section>"));
}

#[test]
fn select_transform_apply_pipeline_supports_javascript_function_rewrite() {
    let file_path = copy_fixture_to_temp("example.js", ".js");
    let select_output = run_identedit(&[
        "select",
        "--kind",
        "function_declaration",
...
#[test]
fn select_transform_apply_pipeline_supports_jsx_function_rewrite() {
    let file_path = copy_fixture_to_temp("example.jsx", ".jsx");
    let select_output = run_identedit(&[
        "select",
        "--kind",
        "function_declaration",
    ...
    let modified = fs::read_to_string(&file_path).expect("file should be readable");
    assert!(modified.contains("<main>Updated</main>"));
}
