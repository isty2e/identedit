...
#[test]
fn select_covers_fish_kinds_and_provider() {
    let fish_file = fixture_path("example.fish");

    assert_select_kind_and_optional_name(&fish_file, "function_definition", Some("process_data"));
    assert_select_kind_and_optional_name(&fish_file, "if_statement", None);
    assert_select_kind_and_optional_name(&fish_file, "command", None);
}

#[test]
fn select_supports_case_insensitive_fish_extension() {
...
fn select_reports_parse_failure_for_syntax_invalid_fish() {
    ...
    assert!(
        !output.status.success(),
        "syntax-invalid fish should fail under the fish provider"
    );
    ...
    assert!(message.contains("tree-sitter-fish"));
    assert!(message.contains("Syntax errors detected in Fish source"));
}

#[test]
fn transform_reports_ambiguous_target_for_duplicate_fish_function_identity() {
    ...
    assert!(
        !output.status.success(),
        "transform should fail for ambiguous duplicate fish function identity"
    );
...
#[test]
fn transform_json_span_hint_disambiguates_duplicate_fish_function_identity() {
    let source = "function configure\n  set value $argv[1]\n  math $value + 1\nend\n\nfunction configure\n  set value $argv[1]\n  math $value + 1\nend\n";
    let file_path = write_temp_source(".fish", source);
    let select_output = run_identedit(&[
        "read",
    ...
    assert!(
        apply_output.status.success(),
        "apply failed after fish span_hint disambiguation: {}",
        String::from_utf8_lossy(&apply_output.stderr)
    );
...
fn select_reports_parse_failure_for_nul_in_fish_source() {
    ...
    assert!(
        !output.status.success(),
        "select should fail for NUL fish source"
    );
...
#[test]
fn apply_reports_parse_failure_for_nul_in_fish_source() {
    let file_path = write_temp_bytes(".fish", b"function run\n  echo ok\nend\0\n");
    let request = json!({
        "files": [{
            "file": file_path.to_string_lossy(),
            "operations": [{
    ...
    assert!(
        !output.status.success(),
        "apply should fail for NUL fish source"
    );
...
#[test]
fn select_supports_fish_switch_statement_kind() {
    let source = "switch $argv[1]\ncase run\n  echo run\ncase '*'\n  echo other\nend\n";
    let file_path = write_temp_source(".fish", source);
    assert_select_kind_and_optional_name(&file_path, "switch_statement", None);
}

#[test]
fn select_ignores_function_like_tokens_inside_comments() {
    ...
    assert!(
        ...
        "function-like comment tokens should not produce function_definition handles"
    );
}
