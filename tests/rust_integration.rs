...
#[test]
fn select_covers_rust_kinds_and_provider() {
    let rust_file = fixture_path("example.rs");

    assert_select_kind_and_optional_name(&rust_file, "function_item", Some("process_data"));
    assert_select_kind_and_optional_name(&rust_file, "struct_item", Some("Processor"));
    assert_select_kind_and_optional_name(&rust_file, "enum_item", Some("Mode"));
    assert_select_kind_and_optional_name(&rust_file, "impl_item", None);
}
...
fn select_transform_apply_pipeline_supports_rust_enum_rewrite() {
    ...
    let modified = fs::read_to_string(&file_path).expect("file should be readable");
    assert!(modified.contains("Turbo"));
}
